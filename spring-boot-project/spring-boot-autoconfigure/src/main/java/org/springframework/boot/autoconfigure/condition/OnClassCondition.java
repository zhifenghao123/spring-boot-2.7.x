...
class OnClassCondition extends FilteringSpringBootCondition {
	...
	/**
	 * OnClassCondition的getOutcomes方法主要解析自动配置类是否符合匹配条件，
	 * 当然这个匹配条件指自动配置类上的注解@ConditionalOnClass指定的类存不存在于classpath中，存在则返回匹配，不存在则返回不匹配。
	 */
	@Override
	...
	/**
	 *
	 * 由于解析自动配置类是否匹配比较耗时，因此从上面代码中我们可以看到分别创建了firstHalfResolver和secondHalfResolver
	...
	private static final class StandardOutcomesResolver implements OutcomesResolver {
		...
		private ConditionOutcome[] getOutcomes(String[] autoConfigurationClasses, int start, int end,
			...
			for (int i = start; i < end; i++) {
				...
				if (autoConfigurationClass != null) {
					...
					if (candidates != null) {
						// 拿到自动配置类注解@ConditionalOnClass的值后，再调用getOutcome方法去判断匹配结果,
						// 若该类存在于类路径，则getOutcome返回null，否则非null
						/*******************【主线，重点关注】******************/
		...
		private ConditionOutcome getOutcome(String candidates) {
			...
			try {
				// 自动配置类上@ConditionalOnClass的值只有一个的话，直接调用getOutcome方法判断是否匹配
		...
		private ConditionOutcome getOutcome(String className, ClassLoader classLoader) {
			...
			/******************【主线，重点关注】********************/
			// 这里调用classNameFilter去判断className是否存在于类路径中，其中ClassNameFilter又分为PRESENT和MISSING两种;
			// 目前只看到ClassNameFilter为MISSING的调用情况，所以默认为true的话记录不匹配信息；
			// 若传入ClassNameFilter为PRESENT的话，估计还要再写一个else分支
